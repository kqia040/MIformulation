...
def potential(V, E_B, E_NB, c_T, pi_R, dist_matrix):
    R = V[0]
    N = V[1]    
    c_X = dict.fromkeys(E_B[1], 0)
    c_NB = dict.fromkeys(E_NB, 0)
...
#            print e, "eeeee"            
            if e[1] is not None:            
                if v in e[0] or v == e[1]:
                    if e[1] is not None:
                        c_T[e] = c_T[e] + pi_R[v]
                    else:
                        c_T[e] = dist_matrix[e[0]] - pi_R[v]
